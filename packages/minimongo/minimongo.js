...
LocalCollection = function (name, isImmutable) {
    var self = this;
    self.name = name;
    self.isImmutable = isImmutable;
    self.IdMap = isImmutable ? LocalCollection._ImmutableIdMap :
        LocalCollection._IdMap;
    // _id -> document (also containing id)
    self._docs = new self.IdMap;
...
_.extend(LocalCollection.Cursor.prototype, {
    ...
    observeChanges: function (options) {
        ...
        var query = {
            ...
            distances: (
                self.matcher.hasGeoQuery() && ordered && new LocalCollection._ImmutableIdMap),
        ...
        if (self.collection.paused)
            query.resultsSnapshot = (ordered ? [] : new LocalCollection._ImmutableIdMap);
...
LocalCollection.Cursor.prototype._getRawObjects = function (options) {
    ...
    // compatible
    var results = options.ordered ? [] : new LocalCollection._ImmutableIdMap;
    ...
    if (self.matcher.hasGeoQuery() && options.ordered) {
        if (options.distances) {
            ...
            distances = new LocalCollection._ImmutableIdMap();
        }
    }
...
LocalCollection.prototype.saveOriginals = function () {
    ...
    self._savedOriginals = new LocalCollection._ImmutableIdMap;
};
